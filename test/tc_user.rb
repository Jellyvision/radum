...
class TC_User < Test::Unit::TestCase
  def setup
    ...
    @u1a_c1_ad1 = ActiveDirectory::User.new("user", @c1_ad1)
    @u1b_c1_ad1 = ActiveDirectory::User.new("user", @c1_ad1)
    @u1c_c2_ad1 = ActiveDirectory::User.new("user", @c2_ad1)
    @u1d_c2_ad1 = ActiveDirectory::User.new("User", @c2_ad1)
    @u2_c3_ad2 = ActiveDirectory::User.new("user", @c3_ad2)
  end
  
  def test_removed_flag_false
    assert(@u1a_c1_ad1.removed == false, "Removed flag should be false")
  end
  
  def test_equal
    assert(@u1a_c1_ad1 == @u1b_c1_ad1, "Should be equal")
  end
  
  def test_equal_container
    assert(@u1a_c1_ad1 == @u1c_c2_ad1, "Should be equal")
  end
  
  def test_not_equal_ad
    assert(@u1a_c1_ad1 != @u2_c3_ad2, "Should not be equal")
  end
  
  def test_equal_common_name_change
    @u1a_c1_ad1.common_name = "Test User"
    assert(@u1a_c1_ad1 == @u1b_c1_ad1,
           "Should be equal with common_name change")
  end
  
  def test_equal_name_case
    assert(@u1c_c2_ad1 == @u1d_c2_ad1, "Should be equal")
  end
  
  def test_add_unix_group_no_exception
    assert_nothing_raised do
      @u1a_c1_ad1.add_group @ug1_c1_ad1
    end
  end
  
  def test_add_unix_group_different_directory_exception
    assert_raise RuntimeError do
      @u1a_c1_ad1.add_group @ug2_c3_ad2
    end
  end
  
  def test_group_add_different_directory_exception
    assert_raise RuntimeError do
      @u1a_c1_ad1.add_group @g3_c3_ad2
    end
  end
  
  def test_add_group_no_exception
    assert_nothing_raised do
      @u1a_c1_ad1.add_group @g1_c1_ad1
    end
  end
  ...
  def test_add_group
    assert_block("Should have added exactly two groups") do
      @u1a_c1_ad1.add_group @g1_c1_ad1
      @u1a_c1_ad1.add_group @g2_c2_ad1
      @u1a_c1_ad1.add_group @g2_c2_ad1
      @u1a_c1_ad1.groups.length == 2 &&
      @u1a_c1_ad1.groups.find { |group| group == @g1_c1_ad1 } &&
      @u1a_c1_ad1.groups.find { |group| group == @g2_c2_ad1 }
    end
  end
  ...
  def test_remove_group_user_removed_from_group
    assert_block("Group should have removed user when removed from user") do
      @u1a_c1_ad1.add_group @g1_c1_ad1
      @u1a_c1_ad1.remove_group @g1_c1_ad1
      ! @g1_c1_ad1.users.find do |user|
        user == @u1a_c1_ad1
      end
    end
  ...
  def test_member_of
    assert_block("User should be member of the single group") do
      @u1a_c1_ad1.add_group @g1_c1_ad1
      (@u1a_c1_ad1.member_of? @g1_c1_ad1) &&
      ! (@u1a_c1_ad1.member_of? @g2_c2_ad1)
    end
  end
end
